...
const ClientContacts = () => {
  ...
  return (
    <section className="min-h-screen px-1 py-2 sm:px-2">
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-indigo-600">
            <RiContactsLine className="text-xl text-white" />
          </div>
          <div>
            ...
            <p className="text-sm text-gray-500">
              Inbox of inbound messages from the contact form
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={loading}
            className="rounded-xl bg-indigo-600 px-4 py-1.5 text-sm font-semibold text-white transition-colors hover:bg-indigo-700 disabled:opacity-50"
          >
            {loading ? "..." : "↻ Refresh"}
          </button>
        </div>
      ...
      {/* Error */}
      {error && (
        <div className="mb-4 flex items-center justify-between rounded-xl border border-red-100 bg-red-50 px-5 py-4 text-sm text-red-600">
          <span>{error}</span>
          <button
            onClick={() => setError("")}
      ...
      {!loading && filteredContacts.length === 0 && (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-gray-200 bg-white py-20 text-center">
          <RiContactsLine className="mb-3 text-5xl text-gray-200" />
          ...
          <p className="mt-1 text-sm text-gray-400">
            {searchTerm
              ...
              : "Contacts will appear here when clients reach out."}
          </p>
        </div>
      )}
      {/* Table */}
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5 whitespace-nowrap">
                        <a
                          href={`tel:${item.phone}`}
                          className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md border border-gray-200 hover:border-indigo-400 hover:bg-indigo-50 text-xs text-gray-700 transition-all"
                        >
                          📞 {item.phone}
                        </a>
                      </td>
