...
const ProcessSection = () => {
  ...
  const steps = [
    { title: "Requirement", desc: "We understand your goals and requirements clearly." },
    { title: "Planning", desc: "We plan the best strategy for design + development." },
    { title: "Design", desc: "Modern UI/UX that matches your brand identity." },
    { title: "Development", desc: "Clean code, fast performance and responsive build." },
      ...
      <div className="max-w-6xl mx-auto mt-12 grid grid-cols-1 md:grid-cols-3 gap-7">
        {steps.map((s, i) => (
          ...
          >
            ...
            <h3 className={`mt-5 text-xl font-bold ${theme === "dark" ? "text-white" : "text-indigo-900"}`}>{s.title}</h3>
            <p className={`mt-2 text-sm ${theme === "dark" ? "text-white/70" : "text-gray-600"}`}>{s.desc}</p>
          </motion.div>
        ))}
      </div>
    </section>
